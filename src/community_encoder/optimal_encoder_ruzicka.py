...
# ============================================================
# 3. Phenological Ruzicka Kernel & Nyström
# ============================================================
def compute_optimal_latent_z_ruzicka(ebird_flat, n_species, n_weeks, latent_dim, n_landmarks=10000, device='cuda'):
    """
    Computes Mercer features using the additive Phenological Ruzicka Kernel.
    
    K_total(x,y) = Sum_over_species( k_s(x,y) )
    k_s(x,y) = Sum_t( min(a,b) ) / Sum_t( max(a,b) )
    ...
    # --- A. Setup ---
    # Reshape to (N, Species, Weeks) for easier slicing
    # We keep data on CPU until needed in batches to save VRAM
    X_all = ebird_flat.reshape(N, n_species, n_weeks)

    # Select Landmarks
    idx_lm = np.random.choice(N, min(N, n_landmarks), replace=False)
    X_lm = X_all[idx_lm] # (M, S, T)
    M = X_lm.shape[0]

    print(f"Computing Exact Kernel on {M} landmarks for {n_species} species...")

    # --- B. Compute K_MM (Landmark vs Landmark) Exactly ---
    # We accumulate the kernel matrix directly on GPU
    K_mm_total = torch.zeros((M, M), device=device, dtype=torch.float32)
    
    # Move Landmarks to GPU (M*S*T floats is usually manageable, ~20k*100*52*4B ~= 400MB)
    try:
        T_lm = torch.tensor(X_lm, device=device, dtype=torch.float32)
    except RuntimeError:
        # Fallback for extreme VRAM constraints: Move species loop to CPU or chunking
        print("VRAM limit warning: Landmarks too large for single tensor. Reduce n_landmarks.")
        return None

    # Loop over species to compute additive kernel
    for s in range(n_species):
        # Slice: (M, T)
        data_s = T_lm[:, s, :]
        
        # Precompute sums over time: (M, 1)
        sum_s = data_s.sum(dim=1, keepdim=True)
        
        # L1 Distance: (M, M) |a - b|
        l1_dist = torch.cdist(data_s, data_s, p=1)
        
        # Terms for Ruzicka
        # sum_min = 0.5 * (sum_x + sum_y - l1)
        # sum_max = 0.5 * (sum_x + sum_y + l1)
        sum_plus_sum = sum_s + sum_s.T
        
        numerator = 0.5 * (sum_plus_sum - l1_dist)
        denominator = 0.5 * (sum_plus_sum + l1_dist)
        
        # Handle Orthogonal Absence: if denominator is 0, sim is 0.
        # Identity: if x=y=0, sum_max=0. Mask zeros to avoid NaN.
        mask = denominator > 1e-6
        k_s = torch.zeros_like(numerator)
        k_s[mask] = numerator[mask] / denominator[mask]
        
        # Add to total master kernel
        K_mm_total += k_s

    # --- C. Eigendecomposition of Master Kernel ---
    # K_total = U Lambda U^T
    # Using cpu() for eigensolver stability on large matrices
    ...
    with torch.no_grad():
        for start in range(0, N, batch_size):
            ...
            # (Batch, S, T)
            batch_np = X_all[start:end]
            T_batch = torch.tensor(batch_np, device=device, dtype=torch.float32)
            B_current = T_batch.shape[0]
            
            # Initialize Kernel block (Batch, M)
            K_batch_lm = torch.zeros((B_current, M), device=device, dtype=torch.float32)
            ...
            for s in range(n_species):
                ...
                sum_b = b_s.sum(dim=1, keepdim=True) # (B, 1)
                sum_lm = lm_s.sum(dim=1, keepdim=True) # (M, 1)
                
                # Rectangular L1 dist (B, M)
                l1_rect = torch.cdist(b_s, lm_s, p=1)
                
                sum_plus_sum = sum_b + sum_lm.T # Broadcast (B, M)
                
                num = 0.5 * (sum_plus_sum - l1_rect)
                den = 0.5 * (sum_plus_sum + l1_rect)
                
                mask = den > 1e-6
                k_s_rect = torch.zeros_like(num)
                k_s_rect[mask] = num[mask] / den[mask]
            ...
            # --- E. Global Rescaling ---
            # Z_target = Z_raw / sqrt(S)
            z_batch = z_batch / np.sqrt(n_species)
...
def compute_kernel_diagnostics_ruzicka(z, ebird_flat, n_species, n_weeks, max_samples=500):
    ...
    NORMALIZED Phenological Ruzicka Kernel (K_true / n_species)
    for a small subset.

    Returns:
        - rmse: absolute RMSE
        - rmse_norm: RMSE normalized by ||K_true||
        - rank_Z: effective rank of the embedding Z
    """
    import torch
    ...
    X_s = torch.tensor(
        ebird_flat[idx].reshape(-1, n_species, n_weeks),
        device=device,
        dtype=torch.float32
    )  # (M, S, T)
    M = len(idx)
    ...
    K_approx = z_s @ z_s.T  # already scaled by 1 / n_species

    # 4. Exact normalized kernel
    K_true = torch.zeros((M, M), device=device, dtype=torch.float32)
    ...
    for s in range(n_species):
        ...
        # IMPORTANT FIX: normalize by number of species
...
def visualize_nystrom_component(Z_k, cmap='viridis', fade_continuous=True, iqr_factor=0.5,
    ...
    """
    Visualize a single Nyström eigenfeature with:
    - Signed percentiles for color
    - Continuous fading (alpha) proportional to magnitude / support
    
    Parameters
    ----------
    Z_k : np.ndarray
        2D array (H x W) of a single component
    cmap : str
        Colormap for visualization
    fade_continuous : bool
        Whether to fade inactive pixels continuously (True) or binary (False)
    iqr_factor : float
        Factor for adaptive support threshold (used only for optional binary fallback)
    title : str
        Optional plot title
    show_colorbar : bool
    ...
    -------
    pct_map : np.ndarray
        Signed percentile map (0–1)
    support_mask : np.ndarray
        Boolean mask indicating pixels above adaptive support
    """
    ...
    # --- 2. Adaptive support (for reference / optional mask) ---
    ...
    if fade_continuous:
        # Compute |Z| percentile within valid pixels
...
if __name__ == "__main__":
    ...
    OUT_DIR = "/home/breallis/dev/range_limits_pymc/misc_outputs/ruzicka_sweep_v1"
            ...
            for dim in sorted(latent_dims):
                ...
                if dim == max_dim:
                    ...
                    meta_out = {
                        ...
                        "kernel": "normalized_additive_ruzicka",
                    }
    ...
    if not df.empty:
        ...
        plt.title('Phenological Ruzicka Kernel: Rank vs Dimension')
        plt.legend()
