...
class GeoCovarianceProbe:
    ...
    def plot_variogram(self, max_dist=200, n_samples=5000):
        ...
        try:
            ...
            pass
        plt.title("Predicted Speed of Ecology: Turnover vs. Distance")
        plt.xlabel("Physical Distance (pixels)")
        plt.ylabel("Predicted Ecological Distance")
...
def metric_loss_preserve_kernel(z, z_target, num_pairs=4096):
    ...
    i, j = idx[0], idx[1]
    dot_pred = (z[i] * z[j]).sum(dim=1)
    with torch.no_grad():
        dot_targ = (z_target[i] * z_target[j]).sum(dim=1)
    kernel_mse = ((dot_pred - dot_targ) ** 2).mean()
    return kernel_mse
...
def train_model(train, val, prism_dim, bui_dim, latent_dim, batch_size=16384, epochs=100, lr=1e-3):
    ...
    for ep in range(1, epochs+1):
        ...
        with torch.no_grad():
            ...
            cos_sim = (z_p * z_t_sub).sum(dim=1).mean().item()
            Kp = z_p @ z_p.T
            Kt = z_t_sub @ z_t_sub.T
            k_mse = ((Kp - Kt)**2).mean()
        ...
        print(f"Ep {ep:03d} | Z_MSE: {val_z_mse:.5f} | CosSim: {cos_sim:.4f} | "
