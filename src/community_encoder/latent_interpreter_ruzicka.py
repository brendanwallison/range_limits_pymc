...
def smooth_abundances_only(ebird_flat, n_weeks, sigma):
    """
    Gaussian temporal smoothing along weeks. Returns flattened N x (S*T) array.
    """
...
if __name__ == "__main__":
    ...
    Z_DIR = "/home/breallis/dev/range_limits_pymc/misc_outputs/ruzicka_sweep_v1/sigma_0.5"
    Z_PATH = os.path.join(Z_DIR, "Z.npy")  # Update to your saved Ruzicka latent
    OUT_DIR = os.path.join(Z_DIR, "ruzicka_sweep_v1/sigma_0.5/latent_audit")
    SIGMA = 0.5  # Temporal smoothing sigma
    ...
    if N_pixels != Z.shape[0]:
        raise ValueError(f"CRITICAL MISMATCH: Z has {Z.shape[0]} rows, but mask has {N_pixels} pixels.")

    # --- Load eBird stack ---
    ebird_stack, meta = load_tifs_structured(DATA_DIR, "*_abundance_median_2023-*.tif")
    H, W, D = ebird_stack.shape
    print(f"Loaded eBird stack: {H} x {W} x {D}")

    # --- Flatten & smooth ---
    ebird_flat_raw = ebird_stack[valid_mask]  # N x D
    ebird_smooth = smooth_abundances_only(ebird_flat_raw, meta['n_weeks'], sigma=SIGMA)

    # --- Run Latent Auditor ---
    latent_auditor = LatentAuditor(Z, ebird_smooth, meta, valid_mask, H, W, OUT_DIR)
    latent_auditor.run_all()  # Generates CSV and heatmaps

    # --- Run Geographic Covariance Probe ---
